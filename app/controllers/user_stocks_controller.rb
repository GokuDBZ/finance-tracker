class UserStocksController < ApplicationController
  ...
  def create
    if params[:stock_id].present?
      @user_stock = UserStock.new(stock_id: params[:stock_id], user_id: params[:user])
    else
      @stock = Stock.find_by_ticker(params[:stock_ticker])
      @user_stock = UserStock.new(stock_id: @stock.id, user: current_user) if @stock.present?
      @stock = Stock.new_from_lookup(params[:stock_ticker]) unless @user_stock.present?
      
      if @stock.present? && @stock.save
         @user_stock  = UserStock.new(stock_id: @stock.id, user: current_user)
      else
        @user_stock = nil 
        flash[:error] = "Stock could not be added"
      end
    end
